...
function CurrentPosition(): React.JSX.Element {
  ...
  if (data.loadStatus.status !== 'ok' || !data.current) {
    const message = 'message' in data.loadStatus ? data.loadStatus.message : 'The selected project could not be loaded.';
    return (
      <main className="screen">
  ...
  return (
    <main className="screen">
      ...
      <p className="lede">One immutable read of the selected project, distilled into the work that is active now.</p>
      ...
      <section className="position-card" aria-labelledby="current-phase-heading">
        ...
        <div className="progress-block">
          <div className="progress-row">
            ...
            <span>{current.progress.percent == null ? 'Not reported' : `${current.progress.percent}%`}</span>
          </div>
        ...
        <dl className="metadata">
          ...
          <div>
            <dt>Phases</dt>
            <dd>
              {current.progress.completedPhases ?? '—'} / {current.progress.totalPhases ?? '—'} complete
            </dd>
          </div>
          <div>
            <dt>Plans</dt>
            <dd>
              {current.progress.completedPlans ?? '—'} / {current.progress.totalPlans ?? '—'} complete
            </dd>
          </div>
          <div>
            <dt>Snapshot read</dt>
