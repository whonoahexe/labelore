...
- **Docs versioning / i18n (Docusaurus-style).** Labelore renders one project's current planning state, not a public docs site with historical published versions in multiple languages. `milestones/` archives already provide the "previous version" concept GSD needs; a full docs-versioning system solves a problem Labelore doesn't have.
...
- **Full activity/notification system (real-time toasts, unread counts, mentions).** Labelore has one user and no live updates in v1; a "recent activity" static list read on load covers the actual need without inventing a notification model with no one to notify.
...
- **Browser-native scroll/back-forward/deep-linkable URLs matter more than custom keybindings here.** Because this is a local web app (not a TUI or Electron shell), the browser's own back/forward, tab restore, and URL-bar history are ergonomics Labelore gets for free *if and only if* every view (a phase, a plan, a search query, a requirement) is a real URL — this is a routing/architecture decision more than a "feature," but it's the single biggest ergonomic win available and it's nearly free if planned for from the start, and expensive to retrofit if the app is built as client-side view-state instead of routes.
...
| Feature | Project dashboards (Linear/Jira/GitHub Projects) | Docs browsers (Docusaurus/MkDocs Material/Obsidian) | Labelore's approach |
|---------|--------------------------------------------------|-------------------------------------------------------|----------------------|
...
- `/home/cinedise/labelore/.planning/PROJECT.md` — confirmed scope, the three pains, out-of-scope list
...
- [Storybook — Sidebar & URLs](https://storybook.js.org/docs/configure/user-interface/sidebar-and-urls) (Story Indexers crawl the filesystem to populate a sidebar — the same CLI-launches-local-browser-UI-over-a-directory pattern Labelore follows)
